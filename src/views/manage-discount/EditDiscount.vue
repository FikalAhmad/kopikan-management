...
const onSubmit = handleSubmit((values) => {
  // discountStore.createDiscount(values)
  console.log('INI VALUES' + values)
})

onMounted(() => {
  const discount = discountStore.discounts?.find((u) => u.id === discountId)
  if (discount) {
    setValues({
      ...discount,
...
<template>
  <Card class="m-5">
    <CardHeader>
      ...
      <CardDescription>Edit discount here</CardDescription>
    </CardHeader>
    <ScrollArea class="h-[450px]">
      <CardContent>
        <form @submit.prevent="onSubmit" class="space-y-4">
          ...
          <!-- Submit -->
          <Button class="w-full" type="submit">Edit Discount</Button>
        </form>
      </CardContent>
    </ScrollArea>
  </Card>
